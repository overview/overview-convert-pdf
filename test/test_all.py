...
def normalize_pdf_bytes(b):
    ...
    b = re.sub(rb'/ID\[<[A-F0-9]{32}><[A-F0-9]{32}>\]', b'ID[<XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX><XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX>]', b)
    return b

def normalize_file_contents(b):
    b = normalize_pdf_bytes(b)
    b = re.sub(rb'\n$', b'', b) # remove final newline -- our test JSON examples often have one
    return b
...
class TestSplitPdfAndExtractText(unittest.TestCase):
    ...
    def assertFragmentEqualsString(self, fragment, name, string):
        self.assertFragmentEqualsBytes(fragment, name, string.encode('utf-8'), 'Wrong contents in fragment {}'.format(name))

    def assertFragmentEqualsFile(self, fragment, testDir, name):
        self.assertFragmentEqualsBytes(fragment, name, read_file_bytes(testDir + '/name'), 'Wrong contents in fragment {}'.format(name))

    def _runAndGatherFragments(self, testDir):
        (retval, stdout, stderr) = run_test_case(testDir)
        self.assertEqual(b'', stderr, 'Got error on stderr: {}'.format(stderr.decode('utf-8')))
        self.assertEqual(0, retval, 'Process did not exit with status code 0. stderr was: {}'.format(stderr.decode('utf-8')))
    ...
    def _expectFragments(self, name, expect, actual):
        ...
        for expect_fragment, actual_fragment in zip(expect, actual):
            self.assertFragmentBytesEqual(expect_fragment.name, expect_fragment.bytes, actual_fragment.bytes)
